...
public class Editing extends GameScene implements SceneMethods {
	...
	private void drawPathPoints(Graphics g) {
		if (start != null) {
			g.drawImage(toolbar.getStartPathImage(), start.getxCord() * 32, start.getyCord() * 32, 32, 32, null);
		}
		if (end != null) {
			g.drawImage(toolbar.getEndPathImage(), end.getxCord() * 32, end.getyCord() * 32, 32, 32, null);
		}
	...
	private void changeTile(int x, int y) {
		if (selectedTile != null) {
			...
			if (selectedTile.getId() >= 0) {
				...
				// Only access lvl[][] when in a new field and or tile is changing
